...
def test_pages():
    return [
        BookPageFactory(title="Very similar to test"),
        BookPageFactory(title="Somewhat similar to test"),
        BookPageFactory(title="Not similar at all"),
    ]
...
def test_index_with_multiple_models():
    ...
    index = registry["AllMediaVectorIndex"]
    index.rebuild_index()

    book_pages_ids = {str(page.pk) for page in book_pages}
    film_pages_ids = {str(page.pk) for page in film_pages}
    found_page_ids = {
        document.metadata["object_id"] for document in index.get_documents()
    }

    assert found_page_ids == book_pages_ids.union(film_pages_ids)

    similar_result = list(index.find_similar(FilmPage.objects.first()))
    assert len(similar_result) > 1
    for p in similar_result:
        assert isinstance(p, (BookPage, FilmPage))
